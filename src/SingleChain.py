...
class SingleChain(object):
    def __init__(self, targets, grids=dict(), initparams=dict(), modelpriors=dict(),
                 tempparams=dict(), existdatapath=False):
        """
        ...
        Parameters:
            ...
            tempparams (dict): Dictionary containing temporary parameters.
        ...
        self.grids, self.priors, self.initparams, self.tempparams = utils.load_params(
            defaults)

        self.grids.update(grids)
        self.initparams.update(initparams)
        self.priors.update(modelpriors)
        self.tempparams.update(tempparams)
        ...
        if not op.exists(savepath) :
            utils.save_config(self.targets, outfile, grids=self.grids, priors=self.priors,
                              initparams=self.initparams, tempparams=self.tempparams)
...
(target.noiseref) if wished differently.'
    ...
    def iterate(self):
        ...
        if proposalmodel is None:
            ...
            logger.debug('iiter %s  Not able to find a proposal for %s' %
                         (self.iiter, modify))
            return
        elif self.iiter < (-self.iter_phase1 + (self.iter_phase1 * 0.01)) and len(proposalmodel[0]) > 5:
            return
        ...
        if np.isnan(self.targets.proposallikelihood):
            logger.debug('iiter %s  Not able to calculate Forward modelling' % (self.iiter))
    ...
    def run_chain(self, chainidx, random_seed=None,  phase=1):
        ...
        self.sampling=Sampling(self.priors, self.grids, self.propdist, swd = self.swd, rf=self.rf, ramod = self.ramod, rstate = self.rstate)

        # Initialize starting model, either from existing model or ramdoly sample 
        tnull0 = gettime.time()
        if self.extract_fromlastmodel_path:

            imodel,iswdnoise, irfnoise, ivpvs, iiter = self.sampling.init_lastmodel_fromchain(self.extract_fromlastmodel_path, chainidx, phase=phase)
        ...
        acceptrate = float(self.n) / current_iterations * 100.
        print(f'FINAL ACCEPT RATE {chainidx} {acceptrate:.4f}')
           
        print(f'FINAL chainidx {chainidx} self.iiter {self.iiter} itrtime {meantime:.4f}')
