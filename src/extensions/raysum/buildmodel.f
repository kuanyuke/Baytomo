...
c Scratch variables:
        integer ilay
c        real rot_axis(3,3),d_a,d_b,AA,CC,LL,NN,FF,a_temp(3,3,3,3)
        real rot_axis(3,3),vpv,vph,AA,CC,LL,NN,FF,a_temp(3,3,3,3)
...
c            d_a=alpha(ilay)*pct_a(ilay)/100.
c            d_b=beta(ilay)*pct_b(ilay)/100.
c            AA=rho(ilay)*(alpha(ilay) - d_a/2.)**2.
c            CC=rho(ilay)*(alpha(ilay) + d_a/2.)**2.
c            LL=rho(ilay)*(beta(ilay) + d_b/2.)**2.
c            NN=rho(ilay)*(beta(ilay) - d_b/2.)**2.
c            FF=eta*(AA-2.*LL)
c            write (*,*) 'Anis. params are:',AA,CC,FF,LL,NN
c ----------
c KY       
            LL=rho(ilay)*((3. * beta(ilay)**2.) / (2. + pct_b(ilay)))
            NN=rho(ilay)*((3. * beta(ilay)**2.) / (1. + 2./pct_b(ilay)))
            vpv=alpha(ilay)**2. + (4./3.)*(LL/rho(ilay)-beta(ilay)**2.)
            vph=alpha(ilay)**2. + (4./3.)*(NN/rho(ilay)-beta(ilay)**2.)
            AA=rho(ilay)*vph
            CC=rho(ilay)*vpv
c            write (*,*) 'Vel ',LL, NN, vpv, vph, AA,CC
c
c KY  
            FF=eta*(AA-2.*LL)
