...
/*                                                                           */
/*       (float)   xs,ys         :   Point source coordinates referred to    */
...
/*          the IEEE standards for INFINITY_FD and related tests. This is non   */
...
#define    INFINITY_FD    0.500e+19
#define    FD_HUGE     0.499e+19
#define    ISINF(x)    ((x)>FD_HUGE)
/* Note: INFINITY_FD must be lower than the SQUARE ROOT of the highest */
/* acceptable float value (machine dependent) to prevent overflow.  */
/* FD_HUGE must be chosen such that INFINITY_FD>FD_HUGE is true.       */
...
/* difference between two "normal" floats encoded on 32 bits */
...
static void
    error(int flag),
    init_nearest(void),
    init_cell(float dx, float dy, int x, int y),
...
static    float     timeshift;              /* see comments: init_point()  */
...
static char    *err_msg[]=
            ...
            "\ntime_2d: [Bug] macros INFINITY_FD, FD_HUGE not properly set.\n",
            ...
            "\ntime_2d: a dimension (nx,ny) is too small or negative.\n",
            };
...
int time_2d_(float *HS, float *T, int *NX, int *NY,
             float *XS, float *YS, float *EPS_INIT, int *MESSAGES)
{   
    int    i,j,signal;
    float  *pf;
...
{    
    ...
    if(!(ISINF(INFINITY_FD))) return ERR_INFBUG;
/* if you encounter this error, it probably means you played */
/* around with the values of macros FD_HUGE and INFINITY_FD !   */
...
/* while keeping these masked values in hs_keep[]. */
    ...
    for(i=0,pf=hs_keep;i<nx;i++,pf++){
        *pf=hs[i][nmesh_y];
        hs[i][nmesh_y]=INFINITY_FD;
    }
    for(j=0;j<nmesh_y;j++,pf++){
        *pf=hs[nmesh_x][j];
        hs[nmesh_x][j]=INFINITY_FD;
    }
...
/* if first pass, assign INFINITY_FD to all times */
        ...
        if(init_stage==0){
            ...
            for(x=0,pf=t_buf;x<ntime;x++) *pf++=INFINITY_FD;
        }
...
static void init_cell(float dx, float dy, int x, int y)
...
/* headwave initialization (returns headwave arrival time or INFINITY_FD) */
...
/* hsc,hsn: hs values in current,neighbour cell                        */
{
    ...
    else return INFINITY_FD;                   /* direct arrival not critical */
}

/*----------------------------------------------------recursive_init()-------*/
...
/* build the re-discretized local model */
    for(i=0;i<N_INIT;i++) HS[i]=T[i]=INFINITY_FD;
    nx=ny=N_INIT_X;
    xs=ys=2*INIT_MIN+1;
    i0=j0=1;
    ihs=xs_-INIT_MIN-1;
