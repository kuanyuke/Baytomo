...
class PlotFromStorage(object):
    ...
    def get_outliers_misfit(self, dev, phase):
        """Detect outlier chains.
        The median likelihood from each chain (main phase) is computed.
        Relatively to the most converged chain, outliers are declared.
        Chains with a deviation of likelihood of dev % are declared outliers.
        ...
        for i, misfile in enumerate(self.misfiles[phase]):
            ...
            chainmisfits = np.loadtxt(misfile)[:,0]
        ...
        # scores must be smaller 1
        scores = chainmedians / minmis - 1
       

        outliers = chainidxs[np.where(((scores) > dev))]
        outscores = scores[np.where(((scores) > dev))]  

        if len(outliers) > 0:
            print('Outlier chains found with following chainindices:\n')
            print(outliers)
            outlierfile = op.join(self.datapath, 'outliers_misfit.dat')
            with open(outlierfile, 'w') as f:
                f.write(
                    '# Outlier chainindices with %.3f deviation condition\n' % dev)
                for i, outlier in enumerate(outliers):
    ...
    def save_final_distribution(self, maxmodels=1000, dev=0.05, thresholds=False, phase= 2):
        ...
        #self.outliers =  self.get_outliers_misfit(dev=dev,phase=phase)
        ...
        chainidxs, nmodels1 = self._get_chaininfo(phase)
        chainidxs2, nmodels2 = self._get_chaininfo(1)
        
        kkk = 0
        for i, cidx in enumerate(chainidxs):
            
    
            p2_path = self.vsfiles[0][i].replace("p1", "p2")
            print (p2_path)
            if os.path.exists(p2_path):
                phase = 1
                nmodels = nmodels2[kkk]
                cidxx = kkk
            else:
                phase = 0
                nmodels = nmodels1[i]
            ...
            if cidxx in self.outliers or cidxx in [19,28] :
                if phase == 1:
                    kkk+=1
                continue

            index = np.arange(nmodels).astype(int)
            if nmodels > mpc:
                if phase == 1:
                    index = rstate.choice(index, mpc, replace=False)
                else:
                    index = list(range(nmodels-mpc-500,nmodels-10))
                    
                index.sort()


            chainfiles = [self.vsfiles[phase][cidxx], self.rafiles[phase][cidxx],self.vonfiles[phase][cidxx],
                          self.misfiles[phase][cidxx], self.likefiles[phase][cidxx], self.swdnoisefiles[phase][cidxx],
                          self.rfnoisefiles[phase][cidxx], self.vpvsfiles[phase][cidxx]]
    ...
    def _plot_iitervalues(self, files, ax, ncells=0, misfit=0, noise=0, ind=-1, noiseind=0,reodercolor=False):
        ...
        print (len(unifiles))
        color_list = base(np.linspace(0, 1, len(unifiles)))# len(unifiles)))
    ...
    def final_gridmodels(self, final=True, chainidx=0, n_jobs = 4, nsample =1000,refinenx=False):
        ...
        outfile= op.join(self.datapath, 'c_deplayers.pkl')
        outfile = op.join( self.datapath, 'c_allgridvs' )
        np.save(outfile, allgridvs)
