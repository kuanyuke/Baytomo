...
class Model(object):
    ...
    def get_stepmodel_from_grids(dz, vp, vs, ra, rho, zmax):
        ...
        _, vsinds = np.unique(vs,return_index=True)
        _, rainds = np.unique(ra,return_index=True)
        
        if vsinds.size ==0 and rainds.size ==0:
            h = np.ones(len(vs)) * dz
            vs_new = vs
            ra_new = ra
            vp_new = vp
            ...
            inds = np.unique(np.concatenate((vsinds,rainds),0)) 
            vs_new = vs[inds]
            ra_new = ra[inds]
            vp_new = vp[inds]
            rho_new = rho[inds]
        ...
        h[-1] = 0
        return h, vp_new, vs_new, ra_new, rho_new
